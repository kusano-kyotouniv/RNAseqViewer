...
sub findfile_rsem(){
	...
	if( -f $rsem_file){
		open FILE, $rsem_file;
		my $line = <FILE>;
		if($line =~ /TPM/){$rsem_file_status = 'available';}else{$rsem_file_status = 'invalid';}
		close FILE;
...
sub draw_align(){
	...
	my $green = $image_align -> colorAllocate(0,255,0);
...
sub draw_scat_background(){
	if($reset_scat_back == 1){if($contigs>0){if($scats>0){
		...
		my $green = $backimage -> colorAllocate(0,255,0);
